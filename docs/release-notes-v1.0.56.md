...
- **New schools**: Southwest Jiaotong University timetable, Guangxi Vocational & Technical University of Agriculture, and Xinjiang University (postgraduate academic system, `xju_post`); plus eight additional WakeUp-compatible alarm protocol families. The directory now covers 340 schools across 31 protocol types.
...
- **Merged import & export entries** — import and export now say "timetable / period table" and share one flow: exporting a period table narrows the format choice to JSON and Sleepy-native, and importing period-table-only content creates just the period table.
...
- **Language selector folded** — the general-settings language card now stays collapsed showing the current language; expanding it reveals all five choices.
...
- Hefei University of Technology (EAMS5) course collection now completes all four stages, and period rows now follow the server-declared layout table, fixing the ~15% period misalignment seen on the Xuancheng campus's 12-period schedule.
- Entering and renaming period tables follows one rule: every timetable and period table name stays unique, with automatic "name 2/3/4" suffixes on copy and import.
...
This release cross-verified against 43 open-source repositories: 11 for the Southwest Jiaotong University timetable, 32 for the WakeUp alarm protocol family (a 33rd was added while cross-checking Xinjiang University), and 11 more for the Hefei University of Technology period-layout fix. The credits page now lists 38 per-school cards and 154 repository entries. Many protocol details were confirmed against their implementations. Thank you.
...
- **新学校**:西南交通大学课表、广西农业职业技术大学、新疆大学(研究生教务 `xju_post`);另新增 WakeUp 兼容闹钟协议族八个学校族。目录现共 340 所、31 类协议。
- **关于页 QQ 交流群入口** —— 一键复制群号并拉起 QQ。
- **学校列表顶部新增自定义 URL 导入位** —— 学校暂未收录时,直接粘贴教务系统地址尝试导入。
- **导入导出合并入口** —— 导入/导出统一为「课表/作息表」表述:导出作息表自动收窄为 JSON 与 Sleepy 原生两种格式;纯作息内容导入只建作息表。
- **全 app 弹窗按钮重设计** —— 色块按钮边界清晰;放得下时排一排,放不下自动改全宽竖排,文字永不截断。
- **语言选择折叠** —— 通用设置语言卡默认收起只显当前语言,展开可见全部五项。
- **非常规时间课的占位节次** —— 窄占位行(如只溢出 5 分钟)渲染为低调灰块;点按展开到正好显示完时间文字,再点折叠。长空隙照常显示时间。

## 优化与修复

- 合肥工业大学(EAMS5)采集四段链补全;节次行改按服务端布局表渲染,修复宣城校区 12 节课表约 15% 的节次错位。
- 课表与作息表名称全域唯一,复制定名时自动顺延「原名2/3/4」。

- 返回课表不再出现周次来回跳变闪烁。
- 新建节次表取消后不再遗留空壳行。
- 草稿恢复保留完整导入配置;草稿箱入口全线接通。
- 多个伙伴协议解析修正经跨仓核验(日期偏移、rowspan、未匹配到课程时的显示)。
- 课程组色与逐卡颜色编辑互不覆盖。
...
本版本经 43 个开源仓库交叉验证:西南交大课表 11 仓、WakeUp 闹钟协议族 32 仓(新疆大学交叉核查时再增 1 仓)、合工大节次布局修复 11 仓。致谢页现有 38 张单校致谢卡、154 条仓库致谢。许多协议细节以其实现为参照确认——感谢他们。
