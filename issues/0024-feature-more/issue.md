...
\n\n### lingion — 2026-09-14 (已发布回复)\n\n两个功能都已经发布了，感谢当时的耐心确认。\n\n**Feature 1（多课表小组件）— 已随 v1.0.52 发布**\n\n按当时商量的方案做成了 app 内管理入口：我的 → 通用设置 →「管理桌面小组件」，页内列出桌面上已放置的每一个小组件，每个实例都可以单独绑定一张课表。\n\n**Feature 2（每日小组件切换日期）— 已随 v1.0.55 发布**\n\n每日 / 双日小组件的日期切换最终做成了底部固定条上的按钮：‹ 前一天 · 回今天 · 后一天 ›。原需求的滑动手势因各启动器兼容性差异改成按钮交互（华为/荣耀/小米/OPPO/vivo/魅族/三星全验证形态）。\n\n附 v1.0.55 小组件重做要点：13 种尺寸五形态、拖拽自动换布局、+N 胶囊计数（双日按列分开）、上下课时间点自动刷新。\n\nURL: https://github.com/lingion/sleepy/issues/24#issuecomment-5672366500
